...
unsigned long set_rep = 0; /* number of bits to represent set number */
unsigned long offset_rep = 0; /*number of bits to represent offset number */
unsigned long tag_rep = 0; /*number of bits to represent tag number */
...
void find_num_sets(){
  num_sets = total_cache_size/(associativity*block_size);  
...
unsigned int num_bits_tag(){
  tag_rep=ADDRS_LENGTH-set_rep-offset_rep; //as tag bits are Address length-set bits-offset bits
  return tag_rep;
...
unsigned long find_set(unsigned long address){
  unsigned long set_num = address>>offset_rep;
  set_num<<=(tag_rep+offset_rep);
  set_num>>=ADDRS_LENGTH-set_rep; //shifting to clear out extra bits
  return set_num;
}
...
unsigned long find_tag(unsigned long address){
  unsigned long tag_num = address>>(offset_rep+set_rep);  //shifting to isolate the tag bits
  return tag_num;
...
unsigned long find_offset(unsigned long address){
  unsigned long offset_num = address<<(ADDRS_LENGTH-offset_rep); //shifting to isolate offset bits 
  offset_num>>=(ADDRS_LENGTH-offset_rep);
  return  offset_num;
